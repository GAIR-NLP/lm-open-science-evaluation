...
def infer(args, test_data):
    ...
    if args.few_shot_prompt is not None:
    ...
    for example in test_data:
        ...
        if args.few_shot_prompt is not None:
            ...
            example['messages'][-2]['content'] = prompt
        prompt = tokenizer.apply_chat_template([example['messages'][-2]], add_generation_prompt=True, tokenize=False)
        prompt = prompt.lstrip()
        if prompt.startswith(tokenizer.eos_token):
    ...
    predictions = [eval(args.answer_extraction_fn)(item) for item in tqdm(test_data, desc="extract answer", total=len(test_data))]
    assert len(model_outputs) > 0, f"{len(model_outputs)}"

    results = []
    for example, pred in zip(test_data, predictions):
...
def main(args):
    random.seed(42)
    # assert args.prompt_format == 'sft', f"this is sft eval. But propmt_format is {args.prompt_format} not sft!"
    print("Loading data...")
    test_data = []
    ...
    with open(os.path.join(args.save_dir, pred_fname), "w", encoding = "utf-8") as fout:
        json.dump(results, fout, ensure_ascii=True)

    metric_fname = "metrics.json"
    if args.n_subsets > 1:
        metric_fname = f"metrics.{args.subset_id}.json"
    with open(os.path.join(args.save_dir, metric_fname), "w", encoding = "utf-8") as fout:
        json.dump({
